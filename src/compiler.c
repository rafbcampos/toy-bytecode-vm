...
static BytecodeSequence *current_sequence() { return compiling_sequence; }
...
static void emit_byte(uint8_t byte) {
  write_bytecode_sequence(current_sequence(), byte, parser.previous.line);
}
...
#ifdef DEBUG_PRINT_CODE
  if (!parser.had_error) {
    disassemble_bytecode_sequence(current_sequence(), "code");
  }
#endif
...
static uint8_t make_constant(Value value) {
  int constant = add_constant(current_sequence(), value);
